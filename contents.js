...
const getFileBlob = async (fileSHA)=> {
  try {
  const response = await fetch(
    `https://api.github.com/repos/${GITHUB_USER}/${GITHUB__REPO}/git/blobs/${fileSHA}`
    );
  const data = await response.json();
...
const convertBlob = async blob => {
  ...
  try {
    ...
    //...... FOR IMAGES ....
    var base64img = `data:image/jpg;base64,${fileContents}`;
    var img = new Image();
    img.onload = function() {
        resolve(img);
    };
...
return utf8Bytes
  ...
  //return utf8Bytes; //custom for images returns the base64 string form of an image the can then be appended using the data urls
  //return atob(utf8Bytes); //original uncomment for files
}
...
function getImageInBase64(imgUrl) {
  ...
  return new Promise((resolve) =>{fetch(imgUrl)
      .then(res=>res.json())
      .then((json)=>{
        utf8Bytes = decodeURIComponent(json.content).replace(/%([0-9A-F]{2})/g, function (match, p1) {
            return String.fromCharCode('0x' + p1);
          });
        resolve(utf8Bytes)
      });
...
/*
  const getFileSHA1 = async () => {
    ...
    getFileBlob1(fileSHA)
  }
  
  const getFileBlob1 = async (fileSHA)=> {
    try {
    const response = await fetch(
      `https://api.github.com/repos/${GITHUB_USER}/${GITHUB__REPO}/git/blobs/${fileSHA}`
      );
    ...
    fileBlob = data.content
    convertBlob1(fileBlob)
    } catch (error) {
      console.log(error);
    }
  }
  
  const convertBlob1 = async blob => {
    //console.log(blob)
    try {
      // const fileContents = Buffer.from(blob, "base64").toString()
      // file = JSON.parse(fileContents)
